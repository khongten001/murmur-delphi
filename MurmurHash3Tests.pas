...
begin
  Log('Using TMurmur1.Hash > Hashing 256 values, followed by hashing the result set of the hashes');
	...
	begin
		key[i]    := Byte(i);
		hashes[i] := TMurmur3.Hash(key[0], i, 256 - i);
	end;
...
var
	...
	begin
		...
		TMurmur3.Hash_x64_128(Pointer(key)^, Length(Key), Seed, @actual);
	...
	begin
		...
		TMurmur3.Hash_x64_128(Pointer(Value)^, Length(Value) * SizeOf(WideChar), Seed, @actual);
